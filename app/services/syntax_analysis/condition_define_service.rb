...
module SyntaxAnalysis
  class ConditionDefineService < StatementService
    ...
    VALID_START_KEYWORDS = ['if', 'elsif', 'else'].freeze
    ...
    def token_for_current_state_valid?(token)
      ...
      if current_state == STATES[:start_condition]
        ...
        return VALID_START_DELIMITERS.include?(token[:lexeme]) if token[:type] == :delimiter
      end
